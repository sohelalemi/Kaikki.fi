(() => {
  ...
  style.textContent = `
    ...
    .housing-back{margin-bottom:12px;background:#eef4ff;color:#1565d8}
    @media(max-width:700px){.subcategory-chips{gap:6px}.subcategory-chips button{font-size:12px;padding:8px 10px}.housing-choice-grid{grid-template-columns:1fr}.housing-choice{min-height:74px;font-size:15px!important}}
  `;
  ...
  function renderHousingTypes(){
    ...
    panel.innerHTML = `<button type="button" class="housing-back">← Takaisin</button><h3>${housingFilter==='Myynti'?'Myytävät asunnot':'Vuokra-asunnot'} – asuntotyyppi</h3><div class="subcategory-chips">${types.map(([name,icon]) => `<button type="button" data-subtype="${name}" class="${subtypeFilter===name?'active':''}">${icon} ${name}</button>`).join('')}</div>`;
    panel.classList.add('show');
    panel.querySelector('.housing-back').onclick=()=>{housingStage='choice';subtypeFilter='';render();};
  ...
  function renderPanel(){
    if (filter === 'Asunnot') {
      if (housingStage === 'types' && housingFilter) { renderHousingTypes(); return; }
      ...
      panel.querySelectorAll('[data-housing-choice]').forEach(btn => btn.onclick = () => {
        housingFilter = btn.dataset.housingChoice;
        housingStage = 'types';
        subtypeFilter = '';
        filter = 'Asunnot';
        render();
  ...
  if(homeSale){const old=homeSale.onclick;homeSale.onclick=e=>{subtypeFilter='';housingStage='types';old?.call(homeSale,e);render();};}
  if(homeRent){const old=homeRent.onclick;homeRent.onclick=e=>{subtypeFilter='';housingStage='types';old?.call(homeRent,e);render();};}
  ...
  form.onsubmit = e => {
    ...
    if (newest && selectedSubtype) {
      ...
      subtypeFilter = selectedCategory === 'Asunnot' ? selectedSubtype : selectedSubtype;
      if(selectedCategory==='Asunnot'){housingStage='types';housingFilter=newest.housingType||'';}
      render();
    }
    syncSubtypeForm();
  };
  ...
  allBtn.onclick = e => { subtypeFilter=''; housingStage='choice'; oldAll?.call(allBtn,e); render(); };
