...
// Hide it while an individual chat is open so the message composer is never covered.
const start=s.lastIndexOf(" <View style={s.bottom}>");
const end=start>=0?s.indexOf('</SafeAreaView>',start):-1;
if(start>=0&&end>start){
 const bottom=` {(['home','notifications','sell','messages','profile','favorites'].includes(tab)&&!(tab==='messages'&&chat))?<View style={{position:'absolute',left:0,right:0,bottom:0,height:88,backgroundColor:'#fff',borderTopWidth:1,borderTopColor:'#e5e7eb',flexDirection:'row',alignItems:'center',justifyContent:'space-around',paddingBottom:8,zIndex:50,elevation:20}}>
