...
function SettingsPressable({originalProps}){
  ...
  const save=(k,setter)=>(v)=>{
    setter(v);
    AsyncStorage.setItem('kaikki.settings.'+k,String(v)).catch(()=>{});
  };

  const toggle=(title,sub,value,onChange)=>h(View,{style:styles.row},
    h(View,{style:styles.rowCopy},
      h(Text,{style:styles.rowTitle},title),
      sub?h(Text,{style:styles.rowSub},sub):null
    ),
    h(Switch,{value,onValueChange:onChange})
  );

  const link=(title,sub,onPress)=>h(Pressable,{style:styles.linkRow,onPress},
    h(View,{style:styles.rowCopy},
      h(Text,{style:styles.rowTitle},title),
      sub?h(Text,{style:styles.rowSub},sub):null
    ),
    ...
    h(Modal,{visible:open,animationType:'slide',onRequestClose:()=>setOpen(false)},
      h(View,{style:styles.page},
        h(View,{style:styles.header},
          h(Pressable,{onPress:()=>setOpen(false),style:styles.back},h(Text,{style:styles.backText},'‹')),
          h(Text,{style:styles.title},'Asetukset'),
          h(View,{style:styles.back})
        ),
        h(ScrollView,{contentContainerStyle:styles.content},
          h(Text,{style:styles.sectionTitle},'Kaikki.fi'),
          h(View,{style:styles.card},
            toggle('Ilmoitukset',notifications?'Käytössä':'Pois käytöstä',notifications,save('notifications',setNotifications)),
            div(),
            link('Kieli',language,()=>setChoice('language')),
            div(),
            link('Teema',theme,()=>setChoice('theme'))
          ),

          h(Text,{style:styles.sectionTitle},'Ilmoitukset'),
          h(View,{style:styles.card},
            toggle('Viestit','Uudet viestit',messages,save('messages',setMessages)),
            div(),
            toggle('Varaukset','Varausten muutokset',reservations,save('reservations',setReservations)),
            div(),
            toggle('Kaikki Diili','Diilin tapahtumat',deals,save('deals',setDeals)),
            div(),
            toggle('Hakuvahdit','Tallennettujen hakujen osumat',searches,save('searches',setSearches))
          ),

          h(Text,{style:styles.sectionTitle},'Yksityisyys'),
          h(View,{style:styles.card},
            toggle('Sijainti','Käytä sijaintia sovelluksessa',location,save('location',setLocation)),
            div(),
            toggle('Näytä puhelinnumero','Muille käyttäjille',phone,save('phone',setPhone)),
            div(),
            toggle('Näytä kaupunki','Profiilissa ja ilmoituksissa',city,save('city',setCity)),
            div(),
            toggle('Online-tila','Näytä milloin olet paikalla',online,save('online',setOnline))
          ),

          h(Text,{style:styles.sectionTitle},'Turvallisuus ja tili'),
          h(View,{style:styles.card},
            link('Vaihda salasana','Päivitä tilisi salasana',()=>Alert.alert('Vaihda salasana','Salasanan vaihto yhdistetään seuraavaksi kirjautumistiliin.')),
            div(),
            link('Kirjaudu ulos kaikilta laitteilta','Suojaa tiliäsi',()=>Alert.alert('Kirjaudu ulos','Haluatko kirjautua ulos kaikilta laitteilta?',[{text:'Peruuta',style:'cancel'},{text:'Jatka',onPress:()=>Alert.alert('Turvallisuus','Toiminto yhdistetään seuraavaksi käyttäjätiliin.')}])) ,
            div(),
            link('Muokkaa tiliä','Nimi, puhelin ja kaupunki',()=>Alert.alert('Muokkaa tiliä','Tilin tietojen muokkaus avataan profiilista.')),
            div(),
            link('Poista tili','Poista tili ja omat tiedot',()=>Alert.alert('Poista tili','Tilin poistaminen on pysyvä toiminto. Varsinainen poistopainike aktivoidaan, kun palvelinpuolen poisto on valmis.'))
          ),

          h(Text,{style:styles.sectionTitle},'Puhelimen asetukset'),
          h(View,{style:styles.card},
            link('Sovellusluvat','Kamera, mikrofoni, sijainti ja ilmoitukset',()=>Linking.openSettings().catch(()=>{}))
          ),

          h(Text,{style:styles.sectionTitle},'Tietoja'),
          h(View,{style:styles.card},
            link('Tietoja sovelluksesta','Kaikki.fi',()=>Alert.alert('Kaikki.fi','Osta, myy ja löydä.')),
            div(),
            link('Käyttöehdot','Palvelun käyttöehdot',()=>Alert.alert('Käyttöehdot','Käyttöehtosivu lisätään ennen julkaisua.')),
            div(),
            link('Tietosuojaseloste','Miten tietojasi käsitellään',()=>Alert.alert('Tietosuoja','Tietosuojaseloste lisätään ennen julkaisua.'))
          )
        )
      )
    ),
...
try{
  ...
  const oj=runtime.jsx;
  const os=runtime.jsxs;
  const wrap=(o,t,p,k)=>shouldPatch(t,p)?o(SettingsPressable,{originalProps:p},k):o(t,p,k);
  if(typeof oj==='function')runtime.jsx=(t,p,k)=>wrap(oj,t,p,k);
  if(typeof os==='function')runtime.jsxs=(t,p,k)=>wrap(os,t,p,k);
...
const styles=StyleSheet.create({
  ...
  sectionTitle:{fontSize:14,fontWeight:'800',color:'#64748b',marginTop:18,marginBottom:8,marginLeft:4},
  ...
  linkRow:{minHeight:70,flexDirection:'row',alignItems:'center',paddingHorizontal:16},
  rowCopy:{flex:1},
  rowTitle:{fontSize:16.5,fontWeight:'700',color:'#111827'},
  ...
  overlay:{flex:1,backgroundColor:'rgba(0,0,0,.35)',justifyContent:'center',padding:28},
  choice:{backgroundColor:'#fff',borderRadius:18,paddingVertical:10},
  choiceTitle:{fontSize:20,fontWeight:'900',padding:18,color:'#111827'},
  choiceRow:{minHeight:56,flexDirection:'row',alignItems:'center',paddingHorizontal:20,borderTopWidth:1,borderTopColor:'#eef0f3'},
  choiceText:{flex:1,fontSize:17,color:'#111827'},
  selected:{fontWeight:'800',color:'#1565d8'},
  check:{fontSize:20,color:'#1565d8'}
});
